...
@then('the new group list is equal to the old one with added group', target_fixture='verify_group_added')
...
def delete_group(app, random_group):
    app.group.delete(group=random_group)

@then('the new group list is equal to the old one without deleted group', target_fixture='verify_group_deleted')
